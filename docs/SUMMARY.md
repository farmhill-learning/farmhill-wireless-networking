# Summary

[Wireless Networking Setup for Farmhill Learning](index.md)
- [The Router](./router.md)
- [Setting up the Router](setup/index.md)
    - [Installing OpenWrt Firmware](setup/installing.md)
    - [Connecting to the Router](setup/connecting.md)
    - [Securing the Device](setup/securing.md)
    - [Connecting to Client Wi-Fi Network](setup/connecting-to-wifi.md)
    - [Enable Wi-Fi Access Point](setup/wifi-access-point.md)
- [Configuring the Disk](disk.md)